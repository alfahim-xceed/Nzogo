...
import { useGetVisaTypeDetailsQuery, useUpdateVisaTypeMutation } from '../../../services/visa_type_api';
...
const UpdateVisaType = () => {
    ...
    const { data: details, isLoading } = useGetVisaTypeDetailsQuery(id);
    ...
    const handleSubmit = async (values, { resetForm }) => {
        try {
            const res = await updateVisaType({ id, name: values.name }).unwrap();
            resetForm();
            toast.success('visa type updated successfully.');
            navigate('/admin/visa-type-list');
        } catch (error) {
            console.error(error);
            toast.error(error.message || 'Failed to update visa type');
        }
    };
