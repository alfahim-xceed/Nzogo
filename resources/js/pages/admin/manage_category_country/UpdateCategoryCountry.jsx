...
import { useGetCategoryCountryDetailsQuery, useUpdateCategoryCountryMutation } from '../../../services/category_country_api';
import { toast } from 'react-toastify';
import { useParams } from 'react-router-dom';

const UpdateCategoryCountry = () => {
    const { data: countries = [] } = useGetCountryListQuery();
    const { data: categories = [] } = useGetVisaCategoryListListQuery();

    const [updateCategoryCountry] = useUpdateCategoryCountryMutation();
    const { id } = useParams();
    const { data: details, isLoading, error } = useGetCategoryCountryDetailsQuery(id);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            await updateCategoryCountry({ id, ...values }).unwrap();
    ...
    return (
        <div className="max-w-lg mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4">Update Category Country </h1>
            <Formik
...
export default UpdateCategoryCountry;
