...
const CategoryCountryRequiredDocumentList=()=>{
    ...
    const handleDelete=async (id)=>{
        try {
            await deleteCategoryCountryRequiredDocument(id).unwrap();
                        ...
                        {details && details.length > 0 ? (
                            details.map((doc, index) => (
                                <tr key={doc.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{doc.required_document.name}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-500 hover:text-blue-700 cursor-pointer">
                                        <Link to={`/admin/manage-category-country-required-documents/update/${doc.id}`}>
...
export default CategoryCountryRequiredDocumentList;
