...
import { useGetCategoryCountryRequiredDocumentDetailsQuery, useUpdateCategoryCountryRequiredDocumentMutation } from '../../../services/category_country_required_document_api';
import { toast } from 'react-toastify';
import { useParams } from 'react-router-dom';

const UpdateCategoryCountryRequiredDocument = () => {
    ...
    const [updateCategoryCountryRequiredDocument] = useUpdateCategoryCountryRequiredDocumentMutation();
    const { id } = useParams();
    const { data: details, isLoading, error } = useGetCategoryCountryRequiredDocumentDetailsQuery(id);

    const initialValues = {
        requirement_document_id: details?.requirement_document_id || '',
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            await updateCategoryCountryRequiredDocument({ id,...values }).unwrap();
    ...
    return (
        <div className="max-w-lg mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4">Update Category Country Required Document</h1>
            <Formik
...
export default UpdateCategoryCountryRequiredDocument;
