...
const CreateVisaCategory = () => {
    ...
    const [createVisaCategory] = useCreateVisaCategoryMutation();

    const handleSubmit = async (values, { resetForm }) => {

        try {
            let { name } = values;
            const res = await createVisaCategory({ name }).unwrap();
            ...
            console.error(error);
            toast.error("Visa category creation error");
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <h1 className="text-2xl font-semibold mb-6 text-gray-800">Create New visa category</h1>
            <Formik
